...
import com.kosta.khn.dao.UserDao;
import com.kosta.khn.dao.UserDaoImpl;
...
//      }
      ...
      response.getWriter().write(dao.idCheck(email)); 
			...
			if(vo==null) {
				...
				session.setAttribute("authUser", vo); //객체 자체를 세션에 세팅
				
				response.sendRedirect("/mysite/main"); //메인으로 돌려보내
				return;
			}
